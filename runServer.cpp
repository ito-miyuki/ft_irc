...
int Server::runServer(Server *server) {
	...
	*/
	...
	serverAddress.sin_port = htons(server->_port); //to convert the port numbers from machine byte order to network byte order.
	...
	*/
	...
	// Step 5: set up Poll (Poll monitors multiple sockets)
	std::vector<pollfd> fds; //pullfd is a struct
	pollfd serverFd;
	serverFd.fd = serverSocket; //サーバーソケットは「新しいクライアント接続」を待つ役割。
	serverFd.events = POLLIN; // // Monitor for incoming connections
	fds.push_back(serverFd);
	/*
		struct pollfd {
			int fd;
			short events; // 監視するイベント (例: POLLIN)
			short revents;// 実際に発生したイベント (poll()で設定される)
		};
